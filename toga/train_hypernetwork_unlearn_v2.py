...
def train(
        ...
        if iter_num % 500 == 0 or iter_num >= max_iter:
            if out_dir:
                ...
                hn_path = os.path.join(
                    out_dir, f"hn-unlearn-p_{p:.2f}-{dataset}-lam_{lam}-{unlearner.method}-v2.pt"
                )
                model_path = os.path.join(
                    out_dir, f"model-unlearn-p_{p:.2f}-{dataset}-lam_{lam}-{unlearner.method}-v2.pt"
                )
    ...
    model_path = os.path.join(out_dir, f"model-unlearn-p_{p:.2f}-{dataset}-lam_{lam}-{unlearner.method}-v2.pt")    
    torch.save(model.state_dict(), model_path)
    hn_path = os.path.join(out_dir, f"hn-unlearn-p_{p:.2f}-{dataset}-lam_{lam}-{unlearner.method}-v2.pt")
    torch.save(hn.state_dict(), hn_path)
